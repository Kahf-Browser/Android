...
/***
    try {
        val posenet = PoseNet.create(context)
        val estimatePoses = posenet.estimatePoses(bitmap)
        Timber.d("psLog estimatePoses: ${estimatePoses.size} ${estimatePoses.firstOrNull()?.keyPoints?.firstOrNull()?.score}")
    } catch (e: Exception) {
        e.printStackTrace()
        Timber.e("psLog estimatePoses failed: $e")
    }
***/

class PoseNet(private val interpreter: Interpreter) :
    PoseDetector {
    ...
    companion object {
        ...
        fun create(context: Context): PoseNet {
