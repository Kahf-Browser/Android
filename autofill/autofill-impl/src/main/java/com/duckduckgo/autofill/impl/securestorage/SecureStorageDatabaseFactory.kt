...
class RealSecureStorageDatabaseFactory @Inject constructor(
    ...
    override fun getDatabase(): SecureStorageDatabase? {
        ...
        // Load SQLCipher native library using ReLinker for reliable loading with App Bundles
        if (!sqlCipherLoaded) {
            try {
                LibraryLoader.loadLibrary(context, "sqlcipher")
                sqlCipherLoaded = true
            } catch (e: Exception) {
                // If loading fails, return null to indicate secure storage is unavailable
                return null
            }
        }
