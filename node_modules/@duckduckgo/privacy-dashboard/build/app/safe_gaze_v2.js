...
async function getImageElements() {
	...
	try {
		...
		const hasMinRenderedSize = (element) => {
			...
			return (
				element.width >= minImageSize && element.height >= minImageSize
			);
		};
		...
		const processImage = async (
			...
			try {
				if (htmlElement.getAttribute("isSent") === type && !srcChanged)
					return;
