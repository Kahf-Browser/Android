// v1.0.4
// Blurring works but has a slight delay
// Copyright 2025 The Kahf Browser Authors. All rights reserved.
...
class SkeletonDrawer {
	...
	drawOvalFaceRegion(ctx, originalCanvas, faceRegion) {
		...
		const gender = faceRegion.gender
        ...
        // Apply grayscale filter for female faces
        if (gender == "female") {
...
function blurImage(image, isInitial = false) {
	...
	if (isInitial) {
		image.style.filter = `blur(8px) opacity(0.5)`;
		return;
	}
...
async function pixelateImage(originalImg, imgData) {
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
            ...
            newImg.src = `data:image/jpg;base64,${imgData}`;
        });
    } catch {
        await new Promise((resolve, reject) => {
            newImg.onload = resolve;
...
// Update unblurImage function to handle original styles
function unblurImage(image) {
    image.dataset.sgIsBlurred = false;
    const ogStyles = image.sgOgStyles;
    image.style.opacity = ogStyles?.opacity || "unset";
    image.style.filter = ogStyles?.filter || "unset";
    image.style.animation = ogStyles?.animation || "none";
	declareSafe(image);
}

function declareSafe (element) {
	if (element.dataset.oldBg) {
		element.style.backgroundImage = element.dataset.oldBg;
	}
}
...
function pixelateRegion(ctx, originalCanvas, width, height) {
   ...
   const pixelSize = Math.max(8, Math.min(
       Math.floor(smallerDimension * 0.08),  // Increased from 0.05
       smallerDimension > 1000 ? 48 : 36     // Increased from 32/24
   ));

   try {
       // Get image data directly from original canvas
       const originalCtx = originalCanvas.getContext('2d', { willReadFrequently: true });
       const imageData = originalCtx.getImageData(0, 0, width, height);
       const pixels = imageData.data;
       const pixelatedData = new Uint8ClampedArray(pixels.length);

       // Pre-calculate block dimensions
       ...
       for (let blockY = 0; blockY < numBlocksY; blockY++) {
           for (let blockX = 0; blockX < numBlocksX; blockX++) {
               ...
               // Sample pixels (using stride for very large blocks)
               const stride = pixelSize > 16 ? 2 : 1;
               ...
               for (let y = startY; y < endY; y++) {
                   for (let x = startX; x < endX; x++) {
                       const i = (y * width + x) * 4;
                       pixelatedData[i] = gray;     // R
                       pixelatedData[i + 1] = gray; // G
                       pixelatedData[i + 2] = gray; // B
                       pixelatedData[i + 3] = pixels[i + 3]; // Keep original alpha
                   }
               }
           }
       }

       // Create the pixelated image data and draw it
       const pixelatedImageData = new ImageData(pixelatedData, width, height);

       // Create a temporary canvas and get the context
       const tempCanvas = createCanvas(width, height);
       const tempCtx = tempCanvas.getContext('2d');

       // Draw the pixelated image data on the temporary canvas
       tempCtx.putImageData(pixelatedImageData, 0, 0);
       ctx.drawImage(tempCanvas, 0, 0);

   } catch (error) {
       // Fallback to simpler method if the optimized approach fails
       console.error("Error in pixelation:", error);

       // Create a single temporary canvas for fallback
       const tempCanvas = createCanvas(width, height);
       const tempCtx = tempCanvas.getContext('2d');
       ...
       try {
           ...
           // Draw original to temp canvas, scaled down then up
           ...
           // Apply grayscale effect with pink tint
           const imageData = tempCtx.getImageData(0, 0, width, height);
           const data = imageData.data;

           for (let i = 0; i < data.length; i += 4) {
               const gray = Math.floor(0.299 * data[i] + 0.587 * data[i + 1] + 0.114 * data[i + 2]);
               data[i] = Math.min(255, gray + 40);     // More red for pink tint
               data[i + 1] = Math.min(255, gray - 20); // Less green
               data[i + 2] = Math.min(255, gray + 10); // Slight blue
           }
...
function pixelateRegion2(ctx, originalCanvas, width, height) {
    ...
    //	const nesrc_temp = tempCtx2.canvas.toDataURL("image/jpg", 0.6);
    //  console.log("nesrc_temp",nesrc_temp)
    ...
    // const nesrc = ctx.canvas.toDataURL("image/jpg", 0.6);
    // console.log("ctx",nesrc)
}
...
function initStyleSheet() {
    ...
    style.innerHTML = `
	.sg-nsfw-blur {
		filter: blur(4px) grayscale(100%) contrast(400%) brightness(100%) contrast(0.5) brightness(300%) !important;
...
async function safegazeOnDeviceModelHandler(uid, detectionResultStr, imgData) {
	...
	try {
		...
		if (isNsfw) {
			if (window.solidColorEffect) {
				blurImage(originalImg);

			} else {
				// pixelate the entire image
				await pixelateImage(originalImg, imgData);
			}
			return;
		}

		if (!Object.keys(detectionResult?.persons ?? {}).length) {
			return
		}
		...
		if (!originalCanvas || !originalCtx) {
			...
			width = originalImg.width || originalImg.clientWidth || originalImg.naturalWidth;
			height = originalImg.height || originalImg.clientHeight || originalImg.naturalHeight;
...
// Apply the mask as a clipping path
            					...
            					const startTimeRegion = performance.now();

//on my phone infinix smart 8 - 15ms
...
// This method is most efficient
                                       // Optimize by scanning rows and creating larger rectangles
                                       for (let y = 0; y < height; y++) {
                                           let startX = -1;

                                           for (let x = 0; x <= width; x++) {
                                               const isPixelMasked = x < width &&
                                                   maskData[(y * width + x) * 4] === 53 &&
                                                   maskData[(y * width + x) * 4 + 1] === 34 &&
                                                   maskData[(y * width + x) * 4 + 2] === 34;

                                               if (isPixelMasked) {
                                                   if (startX === -1) startX = x;
                                               } else if (startX !== -1) {
                                                   // End of a run of masked pixels - add rectangle
                                                   region.rect(startX, y, x - startX, 1);
                                                   startX = -1;
                                               }
                                           }
                                       }
...
// method 2 // This method takes more time

//                                       // Create a binary mask representation
...
//                                           if (contour.length < 3) continue; // Skip invalid polygons
//
...
//                                           ];
//
//                                           // Check if a point is valid and part of the mask
//                                           function isValid(x, y) {
//                                               return x >= 0 && x < width && y >= 0 && y < height &&
//                                                      mask[y * width + x] === 1 && !visited[y * width + x];
//                                           }
//
//                                           // Trace boundary starting from a point
...
//                                               let foundNext = true;
//
//                                               // Simplification: only add points that change direction
//                                               let lastDirection = -1;
//
//                                               while (foundNext) {
//                                                   foundNext = false;
...
//
//                                                       if (isValid(nx, ny)) {
//                                                           // If direction changed or it's the first point
//                                                           if (i !== lastDirection || contour.length === 1) {
//                                                               contour.push({x: nx, y: ny});
//                                                           } else {
//                                                               // Update the last point instead of adding a new one
//                                                               contour[contour.length - 1] = {x: nx, y: ny};
//                                                           }
//
...
//                                               for (let x = 0; x < width; x++) {
//                                                   if (isValid(x, y)) {
//                                                       // Found an unvisited mask pixel - trace its contour
...
//                                       }
  ...
  console.log('Pixelation Region calculation took', performance.now() - startTimeRegion, 'ms');

                    console.log('Starting pixelation process...');
                        const startTime = performance.now();
//                         const nesrc = ctx.canvas.toDataURL("image/jpg", 0.6);
//                        					        console.log("ctx",nesrc)
//                        console.log("original",originalCanvas.toDataURL("image/jpg", 0.6))
					pixelateRegion(ctx, originalCanvas, width, height);
					        console.log('Pixelation process took', performance.now() - startTime, 'ms');
		...
		if (shouldBlur) {
			...
			for (const faceBox of facesToRedraw) {
				skeletonDrawer.drawOvalFaceRegion(ctx, originalCanvas, {
					...
					width: faceBox.width,
					height: faceBox.height,
					gender: faceBox.gender,
				});
			}
			const newSrcUrl = canvas.toDataURL("image/jpg", 0.6);
    ...
    console.log('Total js Took after getting result', performance.now() - totalTime, 'ms');
}
...
async function getImageElements() {
	...
	try {
		const minImageSize = 60; // Minimum image size in pixels
		...
		async function getImageData(src) {
			...
			const imgData = canvas.toDataURL("image/jpg", 0.6).split(",")[1];
		...
		const processImage = async (
			...
			try {
				...
				// Add computed style backup
				const computedStyle = getComputedStyle(htmlElement);
				htmlElement.sgOgStyles = htmlElement.sgOgStyles || {
				...
				if (type === "image") {
					...
					if (hasMinRenderedSize(htmlElement) === false) {
						unblurImage(htmlElement);
						return;
					};
