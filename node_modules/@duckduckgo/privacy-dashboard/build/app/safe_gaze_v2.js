...
function blurImage(image, isInitial = false) {
    ...
    image.classList.add("sg-nsfw-blur"); // Keep using the class-based approach
}
...
function addLoadingEffect (image, type="image") {
	if (type === "image") {
		// if (!["0", 0].includes(image.style.opacity))
        	image.classList.add("sg-loading-effect");
    } else { 
        image.dataset.oldBg = image.sgOgStyles.backgroundImage;
...
function unblurImage(image) {
    image.classList.remove("sg-loading-effect");
	...
	try {
		...
		const processImage = async (
			...
			try {
				...
				addLoadingEffect(htmlElement, type);
