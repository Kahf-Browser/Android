    ...
    analyzeSkeleton(originalImageData, skeletonMask, analysisRegion = 'full') {
      const { width, height, data } = originalImageData;
        ...
        for (let i = 0; i < data.length; i += 4) {
            if (maskData[i] == 53 && maskData[i + 1] == 34 && maskData[i + 2] == 34) { // an exact color that we're using for the detection mask
                totalPixels++;
      ...
      drawSkeleton(ctx, keypoints, mode = 'standard') {
        ...
        this.connections
          ...
          faceKeypoints.forEach(pointName => {
            ...
            if (point) {
              ctx.beginPath();
              ctx.arc(point.x, point.y, config.strokeWidth / 2, 0, 2 * Math.PI);
              ctx.fillStyle = 'red';
              ctx.fill();
            }
        });
    ...
    drawDetectionMask(ctx, keypoints, options = {}) {
        ...
        if (gender === 'male') {
            ...
            lowerBodyConnections.forEach(([startPoint, endPoint]) => {
                ...
                if (start && end && start.score > this.options.minPartScoreMale && end.score > this.options.minPartScoreMale) {
                    ctx.beginPath();
                    ctx.moveTo(start.x, start.y);
                    ctx.lineTo(end.x, end.y);
                    ctx.stroke();
            ...
            if (leftEar && rightEar) {
                const earDistance = (distance || Math.abs(leftEar.x - rightEar.x)) * 1.5;
...
async function safegazeOnDeviceModelHandler(uid, detectionResultStr, imgData) {
    ...
    try { 
        ...
        for (const [key, value] of Object.entries(detectionResult?.persons ?? {})) {
            ...
            const gender = isFemale === null ? '' : isFemale == true? "female": "male";
            ...
            keypoints = keypoints.map(kp => ({
                ...
                const scoreThreshold = ['left_eye', 'right_eye', 'nose', 'left_ear', 'right_ear'].includes(kp.name) 
                ? POSE_DETECTOR_DEFAULTS.minFacePartScore 
                : isMale ? POSE_DETECTOR_DEFAULTS.minPartScoreMale 
            ...
            const analysis = !shouldAnalyzeSkin ? null : skinDetector.analyzeSkeleton(
                imageData,
                maskImageData,
            ...
            if (window.sgSettings?.debug) {
                faceBox && skeletonDrawer.drawFaceDebug(ctx, {
                    x: faceBox.xMin,
                    y: faceBox.yMin,
                    width: faceBox.width,
                ...
                if (analysis) {
                    ...
                    ctx.fillText(`Threshold: ${(gender != "male" ?
...
async function getImageElements() {
    const shouldDebug = localStoreage.getItem('sgDebug') === 'true';
    window.sgSettings = {
        debug: shouldDebug
    };
    try {
