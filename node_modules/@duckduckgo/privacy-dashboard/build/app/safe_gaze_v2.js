...
async function safegazeOnDeviceModelHandler(uid, detectionResultStr) {

    const detectionResult = JSON.parse(detectionResultStr);

    const image = onProcessImageMap.get(uid);
    if (!image) return;
    ...
    for (const [key, value] of Object.entries(detectionResult.persons)) {
        ...
        if (window.sgSettings?.debug) {
            skeletonDrawer.drawFaceDebug(ctx, {
                ...
                isFemale,
                confidence: isFemale ? genderPrediction.femaleConfidence : genderPrediction.maleConfidence
            });
        }
...
class SkeletonDrawer {
    ...
    calculateStrokeWidth(canvasWidth, canvasHeight, mode, gender = "female", keypoints = []) {
        ...
        const multiplier = gender === "female" ? 0.3 : 0.25;
